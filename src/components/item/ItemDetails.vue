<template>
  <div class="row">
    <div :class="{ 'out-of-stock': item.inStock <= 0 }" error-msg="Parduota" class="col-9 d-flex">
      <div class="col-6 p-0">
        <div>
    ...
    <div class="col-3 shadow-sm property__box">
      ...
      <ul class="properties">
        ...
        <li class="property">
          ...
          <span class="property__value">{{ item.size }} / {{ item.width }} / {{ item.height }}</span>
        </li>
        <li class="property">
      ...
      <button
        ...
        v-bind:class="{'btn-primary': isFavored, 'btn-outline-secondary': !isFavored,}"
        @click="
          toggleFavorite({
            ...item,
          })"
      >{{isFavored? 'Atžymėti ★': 'Pažymėti ☆'}}</button>
    </div>
  </div>
</template>
